...
(function () {
  ...
  function renderMatVisual(key) {
    var d = matData[key];
    var img = document.getElementById("matVisualImg");
    ...
    if (img) {
      ...
      setTimeout(function() {
        img.src = images[key] || images["paper"];
        img.alt = (d ? d.name : "Material") + " sample preview";
        img.style.opacity = "1";
      }, 150);
    } else if (svg) {
      var svgs = {
        paper: '<rect width="500" height="600" fill="#F1E2D2"/>',
        vinyl: '<rect width="500" height="600" fill="#2a2d30"/>'
      };
